...
(function () {
  ...
  ocrBtn.addEventListener('click', async () => {
    ...
    try {
      const text = await ScheduleOCR.recognizeScheduleImage(
        photoPreview,
  ...
  ocrParseBtn.addEventListener('click', () => {
    processText(ocrResultText.value);
  });
  ...
  function processText(text) {
    ...
    try {
      ...
      if (!data.shifts || data.shifts.length === 0) {
        showError('Не удалось распознать расписание. Проверьте качество фото или попробуй вставить текст вручную.');
        return;
      }
