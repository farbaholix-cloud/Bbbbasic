...
def ensure_schema(conn):
    ...
    if st_cols and "comment" not in st_cols:
...
.gstep-tx{cursor:pointer}
.gstep-cmt{margin-left:6px;opacity:.4;cursor:pointer;font-size:10px;transition:opacity .12s}
.gstep-cmt:hover,.gstep-cmt.on{opacity:.95}
...
.debt{display:flex;align-items:center;gap:11px;padding:13px 14px;border-radius:15px;margin-bottom:9px;cursor:pointer}
.debt .nm{flex:1}
...
.dbar .col{width:42px;border-radius:11px 11px 6px 6px;display:flex;align-items:center;justify-content:center;transition:height .35s,opacity .3s;box-shadow:inset 0 1px 0 rgba(255,255,255,.25)}
.dbar .col.paid{background:transparent;border:2px dashed rgba(82,224,138,.55);box-shadow:none}
.dbar .col.paid .chk{font-size:19px;color:#52e08a;font-weight:900}
.dbar .col.overdue,.dbar .col.urgent{box-shadow:inset 0 1px 0 rgba(255,255,255,.25),0 0 14px rgba(255,107,125,.35)}
.dbar:hover .col{filter:brightness(1.12)}
.dbar .nm{font-size:9.5px;color:var(--muted);font-weight:700;max-width:58px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center}
.dlegend{display:flex;flex-wrap:wrap;gap:14px;justify-content:center;margin-top:12px;font-size:11px;font-weight:700;color:var(--muted)}
...
.pinc{font-size:10.5px;font-weight:800;padding:5px 9px;border-radius:10px;margin-bottom:7px;cursor:pointer;text-align:center}
.forecast{padding:16px 18px;border-radius:20px}
.forecast .fc-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}
.forecast .fc-h{font-size:15px;font-weight:800}
.forecast .fc-per{display:flex;gap:3px;background:rgba(0,0,0,.25);border:1px solid var(--rim);border-radius:10px;padding:2px}
.forecast .fc-per button{font-size:11px;font-weight:700;color:var(--muted);background:transparent;border:none;padding:5px 12px;border-radius:8px;cursor:pointer}
.forecast .fc-per button.on{color:#fff;background:linear-gradient(135deg,var(--blue),var(--violet))}
.forecast .fc-line{display:flex;justify-content:space-between;align-items:center;gap:10px;font-size:13px;font-weight:600;color:var(--muted);padding:6px 0;line-height:1.3}
...
.forecast .fc-res{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-top:10px;padding:13px 15px;border-radius:14px;font-size:14px;font-weight:800}
.forecast .fc-res b{font-size:19px}
...
.forecast .fc-cov{font-size:11.5px;font-weight:700;color:var(--muted);margin-top:10px;text-align:center}
.forecast .fc-cov b{color:#86b8ff}
.dbar .col.pnow{box-shadow:inset 0 1px 0 rgba(255,255,255,.25),0 0 16px rgba(255,77,94,.5)}
.dbar .col .pico{font-size:16px;filter:drop-shadow(0 1px 2px rgba(0,0,0,.4))}
.dbar .pay-sub{font-size:9.5px;font-weight:800;padding:2px 7px;border-radius:8px;background:rgba(255,255,255,.08);color:var(--muted)}
...
/* Барабаны выбора даты */
.date-roller{display:flex;gap:10px;margin-top:16px}
.date-roller .roller{flex:1;min-width:0;text-align:center;text-align-last:center;padding:14px 6px;border-radius:14px;
  border:1px solid var(--rim);background:rgba(0,0,0,.32);color:var(--txt);font-size:16px;font-weight:800;
...
.arch-proj{margin-bottom:12px;padding:9px;border-radius:12px;background:rgba(0,0,0,.18);border:1px solid rgba(255,255,255,.06)}
...
.arch-ph .arch-act:hover{opacity:1}
.kcard.arch-scard{padding:8px 11px;font-size:11.5px;opacity:.72;background:rgba(255,255,255,.05);cursor:default;margin-bottom:5px}
.kcard.arch-scard.done{text-decoration:line-through;opacity:.45}
...
#page-fin.on>.fin-cards,#page-fin.on>.daysum,#page-fin.on>.fin-full{grid-column:1/-1}
#page-fin.on>.block{grid-column:2}
#page-fin.on>.fin-full{grid-column:1/-1}
...
/* Сетка часов: строка = час, ячейка = день. Час растягивается под самый плотный день,
   события внутри идут друг за другом сверху вниз — без горизонтальных делений. */
.tflow{display:grid;align-items:stretch}
.tfl-lbl{padding:6px 8px 0 0;text-align:right;font-size:10px;font-weight:700;
  color:rgba(235,240,250,.28);border-top:1px solid rgba(255,255,255,.05);
  border-right:1px solid rgba(255,255,255,.07)}
.tfl-lbl.busy{color:#86b8ff;font-weight:900}
.tfl-cell{position:relative;min-height:34px;padding:4px;display:flex;flex-direction:column;gap:4px;
  border-top:1px solid rgba(255,255,255,.05);border-left:1px solid rgba(255,255,255,.06);cursor:pointer}
.tfl-cell:hover{background:rgba(91,157,255,.05)}
.tfl-cell.today{background:rgba(91,157,255,.05)}
.tfl-cell.today.has{background:rgba(91,157,255,.08)}
.t-event{border-radius:8px;padding:5px 8px;font-size:11px;font-weight:700;cursor:pointer;line-height:1.3;
  background:linear-gradient(135deg,rgba(91,157,255,.85),rgba(91,157,255,.5));
  border:1px solid rgba(255,255,255,.2);border-left:3px solid #86b8ff;color:#fff;
  box-shadow:0 2px 8px rgba(0,0,0,.28);flex:0 0 auto;word-break:break-word;
...
.tfl-cell>.t-cursor{left:-1px;right:-1px}
...
<div class="wrap">
  ...
  <div class="page" id="page-fin">
    ...
    <div class="block glass fin-full" id="income-block" style="display:none">
...
// JSON, пригодный для HTML-атрибута: одинарная кавычка в тексте («Café Sa'Sis»)
// обрывала значение onclick, и карточка переставала открываться.
...
function renderProjectBoard(d){
  ...
  // Последний столбец — архив: свёрнутые проекты со своими карточками
...
async function projDelArch(id,name){if(await uiConfirm('Удалить навсегда «'+name+'»?')){mutate(()=>{DATA.archived_projects=(DATA.archived_projects||[]).filter(x=>x.id!==id);},'/api/proj_delete',{id});}}
  ...
  for(const ds of sorted){
    ...
    html+='<div class="cell glass-sm '+(today?'today':'')+' '+(past?'past':'')+'">'+
      ...
      evs.map(e=>{
        ...
        return '<div class="ev '+(e.kind==='reminder'?'rem':'')+'" onclick="openTask('+jsat(tdata)+')">'+
...
function renderFinance(){
  ...
  renderDebtChart(d);
  document.getElementById('cur-cnt').textContent=cur.length?cur.length+' · '+eur(cur.reduce((a,b)=>a+_rem(b),0)):'нет';
...
function renderDebtChart(d){
  const block=document.getElementById('debtchart-block');if(!block)return;
...
function _payNext(p){
  ...
  if(p.recur==='weekly'){
    const target=((p.day||0)%7);const curMon=(now.getDay()+6)%7;const diff=(target-curMon+7)%7;
    const d=new Date(now);d.setDate(now.getDate()+diff);return d;
  }
  const day=Math.min(Math.max(p.day||1,1),31);
  const y=now.getFullYear(),m=now.getMonth();const dimThis=new Date(y,m+1,0).getDate();
...
function _payInfo(p){
  const next=_payNext(p);let days=null;
  if(next){const now=new Date();now.setHours(0,0,0,0);days=Math.round((next-now)/864e5);}
  let urg='far';if(days!=null)urg=days<=2?'now':days<=7?'soon':days<=15?'mid':'far';
...
function openPayment(id){
  ...
  const sub=when+(i.days!=null?' · '+(i.days<=0?'сегодня':i.days===1?'завтра':'через '+i.days+' дн'):'')+' · '+eur(p.amount||0);
  const {bg,sheet}=_openSheet(
    '<div class="grab"></div>'+
    '<div class="stitle">'+(p.icon||'💸')+' '+esc(p.title)+'</div>'+
    ...
    '<button class="sh-btn danger" id="pay-del">🗑 Удалить платёж</button></div>','top-left');
...
function renderIncomeChart(d){
  ...
  document.getElementById('incomechart').innerHTML=sorted.map(p=>{
    ...
    const h=Math.round(28+((p.expected_income||0)/maxA)*112);
    const dnum=_incDays(p);const dlabel=dnum==null?'—':(dnum<=0?'ждём':dnum+'д');
...
function renderForecast(d){
  ...
  let needPay=0;pays.forEach(p=>{const i=_payInfo(p);if(i.days!=null&&i.days<=period)needPay+=(p.amount||0);});
  let needDebt=0;debts.forEach(x=>{const u=_debtUrg(x);if(!u.paid&&u.days!=null&&u.days<=period)needDebt+=_rem(x);});
  const need=needPay+needDebt;const bal=d.balance||0;
  const projected=bal+expW-need;const ok=projected>=0;
...
function projIncome(id){
  ...
  const {bg,sheet}=_openSheet(
    ...
    (cur?'<div class="addr" onclick="projIncomeClear('+id+')" style="cursor:pointer;color:#ff9aa6;text-align:center">убрать ожидаемый профит</div>':''),'top-left');
  ...
  sheet.querySelector('#pi-date').onclick=async()=>{
    ...
    if(r===undefined){projIncome(id);return;}
...
function uiDate(title,curISO,opts){
  ...
  return new Promise(resolve=>{
    ...
    function fixDays(){const dim=new Date(+ySel.value,+mSel.value+1,0).getDate();const cur=Math.min(+dSel.value||1,dim);let h='';for(let dd=1;dd<=dim;dd++)h+='<option value="'+dd+'" '+(dd===cur?'selected':'')+'>'+dd+'</option>';dSel.innerHTML=h;}
...
function openDebt(id){
  ...
  const sub=u.paid?'✅ погашен полностью':('осталось '+eur(u.rem)+' из '+eur(x.total)+(u.pillText?' · '+u.pillText:''));
  const {bg,sheet}=_openSheet(
    '<div class="grab"></div>'+
    '<div class="stitle">'+(x.icon||'💳')+' '+esc(x.name)+'</div>'+
    '<div class="ssub">'+sub+'</div>'+
    '<div class="lbar" style="margin:10px 0 4px"><div class="lfill" style="width:'+pct+'%"></div></div>'+
    (u.paid?'':'<div class="sh-comment-wrap" style="margin-top:12px"><div class="sh-comment-lbl">↩️ Возврат</div>'+
      '<div class="fin-row" style="display:flex;gap:8px"><input class="fin-inp" id="debt-ret" type="number" inputmode="decimal" placeholder="сумма € (осталось '+eur(u.rem).replace(/\s?€/,'')+')" style="flex:1"/>'+
      '<button class="fbtn in" id="debt-ret-go" style="flex:0 0 auto;padding:0 16px">записать</button></div></div>')+
    '<div class="sh-actions" style="flex-direction:column;gap:8px;margin-top:12px">'+
    ...
    '<button class="sh-btn danger" id="debt-del">🗑 Удалить долг</button></div>','top-left');
...
function calAddEvent(e,ds,hour){
