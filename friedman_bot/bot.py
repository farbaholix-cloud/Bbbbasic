...
def war_risk_signals_sync() -> dict:
    ...
    prompt = (
        ...
        "прямого военного конфликта России со странами ЕС/НАТО за последние ~2 недели. "
        ...
        "Источники в приоритете: nato.int, bundesregierung.de, bmvg.de, bundeswehr.de, tagesschau.de, "
        "consilium.europa.eu, официальные сайты разведок и МО стран ЕС, Reuters, FAZ, Zeit. "
        "Государственные СМИ воюющих сторон источником факта не считаются: их сообщения можно приводить "
        "только как заявление, с указанием, кто это заявил.\n\n"
        "ЖЁСТКИЕ ПРАВИЛА. Не выдумывай событий, дат и цитат. Каждый пункт должен опираться на конкретный "
        "материал с датой. Не пересказывай слухи, анонимные источники и «эксперты считают». Отделяй факт "
        "от оценки. Если за период существенно ничего не менялось — так и напиши, это нормальный ответ; "
        "выдумывать напряжение, чтобы было о чём написать, категорически нельзя.\n\n"
        "Верни СТРОГО JSON без пояснений: "
        '{"level":"...","summary":"...","changed":"...","importance":"...","confidence":"...","date":"..."}. '
        ...
        "подготовка — государства ЕС делают практические шаги: законы, гражданская оборона, закупки. "
        "тревога — прямые военные действия против страны НАТО, статья 4/5, эвакуационные меры. "
        "summary — 2–4 очень коротких факта, по-русски, через « · » как разделитель, каждый с источником "
        "в скобках, например «(Bundeswehr, 3.09)». Без воды и без эпитетов. "
        "changed — одной фразой: что изменилось по сравнению с прошлыми неделями; если ничего — «без изменений». "
        "importance — одной фразой и спокойно: значит ли это что-то практическое для жителя Франкфурта "
        "с малым бизнесом прямо сейчас. Честно «на повседневную жизнь пока не влияет», если так и есть. "
        "confidence — «высокая/средняя/низкая» + кратко почему (официальный источник или пересказ СМИ). "
        "date — период, за который собраны новости."
    )
    try:
        result = _claude_exec([CLAUDE_BIN, "-p", prompt,
             "--allowedTools", "WebSearch,WebFetch",
...
async def render_owner_brief(bot, chat_id: int, verbose: bool = False):
    ...
    if war_risk.get("summary"):
        lvl = war_risk.get("level") or "внимание"
        dot, _ = WAR_LEVELS.get(lvl, WAR_LEVELS["внимание"])
        lines.append(f"\n📡 *РФ — ЕС: признаки эскалации* · {dot} {lvl}\n{war_risk['summary']}")
        if war_risk.get("changed"):
            lines.append(f"_Изменение: {war_risk['changed']}_")
        if war_risk.get("importance"):
            lines.append(f"_Тебе: {war_risk['importance']}_")
        conf = war_risk.get("confidence")
        if conf:
