...
Шифрование — только стандартная библиотека (без pip install): scrypt(пароль) →
ключи, гамма HMAC-SHA256 в режиме счётчика, подпись encrypt-then-MAC. Файл
остаётся текстовым: шапка открытым текстом (что это, когда, чем расшифровать) +
тело в base64. Подробности схемы — в комментарии перед _keys_from_pass().
...
def render_markdown(conn):
    ...
    # ── прочий доход на проверку ──
    rev = fc.income_review(conn)
    if rev:
        A("## Прочий доход — проверить, не клиент ли это (%d)" % len(rev))
        A("")
        A("Эти приходы банк отнёс к «прочему доходу», поэтому в выручку они НЕ входят.")
        A("")
        A("| Дата | Сумма | Контрагент |")
        A("|---|---:|---|")
        for r in rev:
            A("| %s | %s | %s |" % (r["date"], _eur(r["amount"]), (r["party"] or "").replace("|", "/")))
        A("")
