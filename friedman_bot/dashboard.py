...
def ensure_schema(conn):
    ...
    # invoice_archive: наполняется ботом Юриста (общая БД). Здесь только гарантия схемы
    # и флаг paid — «оплачено» (новые инвойсы в архив по умолчанию считаются оплаченными).
...
.debtchart .runout{width:1px;flex:0 0 1px;align-self:stretch;background:#ff4d5e;box-shadow:0 0 6px rgba(255,77,94,.8);margin:0 4px;position:relative}
.debtchart .runout::before{content:'💸';position:absolute;top:-2px;left:50%;transform:translateX(-50%);font-size:11px}
...
function renderPaymentChart(d){
  ...
  // точка исчерпания денег на карте нарастающими платежами (без учёта поступлений)
...
// Клик по фрейму баланса (нал/карта/всего) — быстрое изменение: приход, расход
// или установка актуального баланса. Внешний вид фреймов не меняется.
